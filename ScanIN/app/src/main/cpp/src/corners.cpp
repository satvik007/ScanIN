...
int order_points (std::vector < cv::Point > &pts, std::vector < cv::Point > &rect) {
    rect.resize (4);
    // top left
    rect[0] = *min_element (pts.begin(), pts.end(), [&] (cv::Point &x, cv::Point &y) {
        return x.x + x.y < y.x + y.y;
    });
    // bottom right
    rect[2] = *max_element (pts.begin(), pts.end(), [&] (cv::Point &x, cv::Point &y) {
        return x.x + x.y < y.x + y.y;
    });
    // top right
    rect[1] = *min_element (pts.begin(), pts.end(), [&] (cv::Point &x, cv::Point &y) {
        return x.y - x.x < y.y - y.x;
    });
    // bottom left
    rect[3] = *max_element (pts.begin(), pts.end(), [&] (cv::Point &x, cv::Point &y) {
        return x.y - x.x < y.y - y.x;
    });
    for (int i = 0; i < 4; i++) {
        for (int j = i + 1; j < 4; j++) {
